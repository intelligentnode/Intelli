...
class NvidiaWrapper:
    def __init__(self, api_key: str, base_url: str = None, timeout: int = 180):
    ...
    def generate_text_stream(self, params: dict):
        params["stream"] = True
        url = self.base_url + self.chat_endpoint
        response = requests.post(url, json=params, headers=self.headers, stream=True, timeout=self.timeout)
        response.raise_for_status()
